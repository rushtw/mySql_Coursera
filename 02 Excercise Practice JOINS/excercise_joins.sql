...
SELECT c.FullName, c.PhoneNumber, 
	b.BookingDate, b.NumberOfGuests FROM customers AS c 
INNER JOIN bookings AS b ON c.CustomerID=b.CustomerID;

-- left is "customers LEFT JOIN bookings"
SELECT customers.CustomerID, bookings.BookingID FROM
customers LEFT JOIN bookings 
ON bookings.CustomerID=customers.CustomerID;

-- left is "bookings LEFT JOIN customers"
SELECT customers.CustomerID, bookings.BookingID FROM
bookings LEFT JOIN customers 
ON bookings.CustomerID=customers.CustomerID;


-- Question 1
-- The Little Lemon database contains two tables: 
-- “Customers” and “Bookings”. 
-- “CustomerID” is the common column between both tables. 
-- Write the missing JOIN type to extract the data of 
-- all customers who made bookings.
