...
-- Task 1: write string functions to list items, quantities and order status in the following format:
-- Item name–quantity–order status
-- Item name should be in lower case. Order status should be in upper case.
select CONCAT (UCASE(name),"-",quantity,"-",LCASE(orderStatus)) as my_conct
from item,mg_orders
...
select orderid,round(0.05*cost,2) as HANDLING from mg_orders;
