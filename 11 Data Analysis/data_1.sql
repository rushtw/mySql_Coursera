...
-- Task 1
-- Lucky Shrub need to find out how many sycamore trees they’ve sold over the past few years. 
...
select YEAR(orders.Date),sum(quantity) as 'P4 quantity sold ' from orders inner join products
on orders.productid=products.productid where products.productid='p4' and YEAR(orders.Date)>=2020 and YEAR(orders.Date)<=2022 group by YEAR(orders.Date);
...
select clients.clientID,contactNumber from clients inner join orders on clients.clientID=orders.clientID;

select clients.clientID,contactNumber,orders.Date from clients inner join orders on clients.clientID=orders.clientID where YEAR(orders.Date)=2021 OR YEAR(orders.Date)=2022;

select clients.clientID,contactNumber,orders.Date from clients inner join orders on clients.clientID=orders.clientID where YEAR(orders.Date)=2021 OR YEAR(orders.Date)=2022;

-- inner join on 3 tables
select clients.clientID,contactNumber,addresses.street,addresses.county,orders.orderid,orders.productid,orders.Date from clients inner join orders on clients.clientID=orders.clientID inner join addresses on clients.addressid=addresses.addressid 
where YEAR(orders.Date)=2021 OR YEAR(orders.Date)=2022;

-- inner join on 4 tables
select clients.clientID,contactNumber,addresses.street,addresses.county,orders.orderid,products.productid,products.productname,orders.cost,orders.Date 
from clients inner join orders on clients.clientID=orders.clientID inner join addresses on clients.addressid=addresses.addressid inner join products on products.productid=orders.productid 
where YEAR(orders.Date)=2021 OR YEAR(orders.Date)=2022 ORDER BY orders.Date;


-- Task 3
-- Lucky Shrub needs to analyze the sales performance of their Patio slates product in the year 2021. 
-- This product has a Product ID of P3. 
-- Help Lucky Shrub to analyze the performance of this product by developing a function called FindSoldQuantity that enables them to:
...
select sum(quantity) from orders where productid='p3' and year(date)=2021; 

delimiter $$
create function FindSoldQuantity(productid_par VARCHAR(10), year_par int)
returns int deterministic 
begin
declare total_quant int;
select sum(quantity) from orders where productid=productid_par and year(date)=year_par INTO total_quant;
return total_quant;
end $$
