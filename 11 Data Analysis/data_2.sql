...
select avg(cost) from orders where YEAR(date)=2022;
...
DETERMINISTIC
BEGIN
    DECLARE avg_cost DECIMAL(6,2);
    select avg(cost) from orders where YEAR(date)=year_par INTO avg_cost;
    RETURN avg_cost; 
END$$
DELIMITER ;
...
-- outputs the total number of items sold during the last three years for the P1 Product ID. Input the ProductID when invoking the procedure.
select sum(quantity) from orders where productid='p1' and YEAR(date)=2020;

DELIMITER //

CREATE PROCEDURE EvaluateProduct(IN productid_par VARCHAR(10), IN year_1 INT, IN year_2 INT, IN year_3 INT, OUT x1 INT, OUT x2 INT, OUT x3 INT )
BEGIN
	select sum(quantity) INTO x1 from orders where productid=productid_par and YEAR(date)=year_1;
    select sum(quantity) INTO x2 from orders where productid=productid_par and YEAR(date)=year_2;
    select sum(quantity) INTO x3 from orders where productid=productid_par and YEAR(date)=year_3;
END //

DELIMITER ;

CALL EvaluateProduct('p1', 2020, 2021, 2022, @x1, @x2, @x3);
select @x1, @x2, @x3;
...
ON Orders 
FOR EACH ROW 
INSERT INTO Audit (OrderDateTime) -- must specify column
VALUES (Current_timestamp);

INSERT INTO orders VALUES  (31,"Cl1", "P1", 100,5000, "2025-09-01");
...
select fullname, addresses.street, addresses.county from clients inner join addresses 
on clients.addressID=addresses.addressID 
order by addresses.street;

-- inner joing of t3 tables won't work because I want tfullname to be in one column etc...
...
on clients.addressID=addresses.addressID 
UNION
select fullname, addresses.street, addresses.county from employees inner join addresses 
on employees.addressID=addresses.addressID
order by street; -- ORDER BY cannot be used
...
select sum(quantity) from orders where productid='p2' group by YEAR(date);

select sum(quantity) from orders where productid='p2' AND YEAR(date)=2020
union
select sum(quantity) from orders where productid='p2' AND YEAR(date)=2021
union
select sum(quantity) from orders where productid='p2' AND YEAR(date)=2022;
...
select buyprice, sellprice, sellprice-buyprice as profit_per_item, orders.quantity, (sellprice-buyprice)*orders.quantity as total_profit from products 
INNER JOIN orders 
on products.productID=orders.productID
where products.productid="p1" and YEAR(orders.date)=2020;


select sum((sellprice-buyprice)*orders.quantity) from products 
INNER JOIN orders 
on products.productID=orders.productID
where products.productid="p1" and YEAR(orders.date)=2020;

-- put this in a PROCEDURE
drop procedure if exists GetProfit;

DELIMITER //

CREATE PROCEDURE GetProfit(
	IN productid_par VARCHAR(10), IN year_par INT, OUT total_profit_par decimal(6,2)
)
BEGIN
	select sum((sellprice-buyprice)*orders.quantity) as total_profit from products 
		INNER JOIN orders 
		on products.productID=orders.productID
		where products.productid=productid_par and YEAR(orders.date)=year_par INTO total_profit_par;
END //

DELIMITER ;

CALL GetProfit('p1',2020,@total_profit);
SELECT @total_profit;


-- class solution
drop procedure if exists GetProfit_2;

DELIMITER //
CREATE PROCEDURE GetProfit_2(IN product_id VARCHAR(10), IN YearInput INT)
BEGIN
...
-- Help them out by creating a virtual table called DataSummary that joins together the four tables that contain this data. These four tables are as follows:
...
select clients.fullname,clients.contactnumber,productName,addresses.county,products.productID,orders.cost, YEAR(orders.date) from products inner join orders 
on products.productID=orders.productID 
inner join clients on
clients.clientID=orders.clientID
inner join addresses on
addresses.addressid=clients.addressid
...
CREATE TEMPORARY TABLE DataSummary 
select clients.fullname,clients.contactnumber,productName,addresses.county,products.productID,orders.cost, YEAR(orders.date) from products inner join orders 
on products.productID=orders.productID 
inner join clients on
clients.clientID=orders.clientID
...
drop view if exists DataSummary_1;

 CREATE VIEW DataSummary_1 AS 
 ...
 ON Clients.AddressID = Addresses.AddressID INNER JOIN Orders 
 ON Clients.ClientID = Orders.ClientID INNER JOIN Products 
 ON Orders.ProductID = Products.ProductID WHERE YEAR(Orders.Date) = 2022 
 ORDER BY Orders.Cost DESC;
 
 select * from DataSummary_1;
